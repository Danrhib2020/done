...
local blipcamarones = AddBlipForCoord(Config.camarones.x, Config.camarones.y, Config.camarones.z)
			...
			SetBlipScale  (blipcamarones, 0.8)
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
			...
			for k in pairs(locations) do
				if GetDistanceBetweenCoords(locations[k].x, locations[k].y, locations[k].z, GetEntityCoords(GetPlayerPed(-1))) < 150 then			
					...
					if GetDistanceBetweenCoords(locations[k].x, locations[k].y, locations[k].z, GetEntityCoords(GetPlayerPed(-1)), false) < 2 then			
						DrawText3Ds(vector3(locations[k].x, locations[k].y, locations[k].z), "Press [E] to catch crabs/shrimp ", 0.35)
			...
			if GetDistanceBetweenCoords(Config.Processing.x, Config.Processing.y, Config.Processing.z, GetEntityCoords(GetPlayerPed(-1))) < 2 then			
					...
					if GetDistanceBetweenCoords(Config.Processing.x, Config.Processing.y, Config.Processing.z, GetEntityCoords(GetPlayerPed(-1)), true) < 2 then			
						...
						if IsControlJustReleased(0, Keys['E']) then
							local finished = exports['pyrp_skillbar']:skillBar(15000, math.random(3, 5))
							if finished <= 0 then
								--TriggerEvent('esx_status:add', 'stress', 8000)
								exports['mythic_notify']:DoLongHudText('error', 'failed')
							else
								if exports["pyrp_inventory"]:hasEnoughOfItem('crab', 10, false) then
									TriggerEvent("inventory:removeItem", 'crab', 10)
									TriggerEvent('player:receiveItem', 'crab_box', 1, false)
								else
									exports['mythic_notify']:DoLongHudText('error', 'You don\'t have enough crab, minimum of 10 crab is needed.')
								end
							end
						end
					end
...
------------------------------------------------------------------------------------------------------------------------------------------------------
				...
				if GetDistanceBetweenCoords(Config.camarones.x, Config.camarones.y, Config.camarones.z, GetEntityCoords(GetPlayerPed(-1))) < 2 then			
					...
					if GetDistanceBetweenCoords(Config.camarones.x, Config.camarones.y, Config.camarones.z, GetEntityCoords(GetPlayerPed(-1)), true) < 2 then			
						...
						if IsControlJustReleased(0, Keys['E']) then
							...
							if finished <= 0 then
								...
								if exports["pyrp_inventory"]:hasEnoughOfItem('shrimp', 10, false) then
									TriggerEvent("inventory:removeItem", 'shrimp', 10)
									TriggerEvent('player:receiveItem', 'shrimp_box', 1, false)
								else
									exports['mythic_notify']:DoLongHudText('error', 'You don\'t have enough shrimp, minimum of 10 shrimp is needed.')
								end
							end
						end
					end
...
function Animacion()
	...
	local finished = exports['pyrp_skillbar']:skillBar(3000, math.random(3, 5))
...
function Process()
	...
	local making = true
		...
		TriggerEvent('esx:showNotification', '~g~Comenzando ~w~la produccion ~g~de cangrejos')
		Citizen.Wait(5000)
...
local makingcamarones = true
	...
	TriggerEvent('esx:showNotification', '~g~Comenzando ~w~la produccion ~g~de camarones')
	Citizen.Wait(5000)
